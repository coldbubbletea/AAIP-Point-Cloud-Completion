...
def test_emd():
    x1 = torch.rand(32, 8192, 3, device='cuda', requires_grad=True)
    # x1 = x1.cuda() # please normalize your point cloud to [0, 1]
    x2 = torch.rand(32, 8192, 3, device='cuda', requires_grad=False)
    y1 = x1.clone().detach().requires_grad_(True)
    y1 = y1 * 2
    # x_copy = x1.clone()
    # x1 = x1 * 2
    # x_copy = x_copy * 2
    ...
    sia_assignment, auc_assignment, sia_value = sia_module.sia(x1.cpu(), x2.cpu(), 3000)
    # print(x1.device, x2.device)
    sia_x2 = sia_module.assign_points(x2, sia_assignment)
    sia_dist = sia_module.cal_dist(x1[:, 4096:, :], sia_x2)
    # print(type(sia_assignment), type(auc_assignment), type(sia_value))
    auc_x2 = sia_module.assign_points(x2, auc_assignment)
    auc_dis, _ = emd(x1[:, :4096, :], auc_x2, 0.005, 50)
    auc_dist = torch.sqrt(auc_dis).mean(-1)

    loss = sia_dist.mean() + auc_dist.mean()
    loss.backward()
    print(x1.grad)
    ...
    # x3 = np.take_along_axis(auc_x2.cpu(), assigment, axis = 1).cuda()
    # d = (x1[:, :4096, :] - x3) * (x1[:, :4096, :] - x3)
